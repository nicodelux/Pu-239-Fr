...
if (XBT_TRACKER) {
    ...
    $htmlout .= "
                <li>
                    <a href='{$site_config['baseurl']}/mybonus.php'>
                        <span class='button tag is-success dt-tooltipper-large' data-tooltip-content='#karma_tooltip'>Contribution du Freeleech</span>
                        <div class='tooltip_templates'>
                            <span id='karma_tooltip'>
                                <div class='size_4 has-text-centered has-text-success has-text-weight-bold bottom10'>Contribution du Freeleech</div>
                                <div class='level is-marginless'>
                                    <span>Freeleech</span><span> [ ";
    if ($freeleech_enabled) {
        $htmlout .= "<span class='has-text-success'> ACTIF </span>" . get_date($freeleech_start_time, 'DATE') . ' - ' . get_date($freeleech_end_time, 'DATE');
    } else {
        $htmlout .= $fstatus;
    }
    $htmlout .= ' ]
    ...
    $htmlout .= "
                                <div class='level is-marginless'>
                                    <span>Upload X2</span><span> [ ";
    if ($double_upload_enabled) {
        $htmlout .= "<span class='has-text-success'> ACTIF </span>" . get_date($double_upload_start_time, 'DATE') . ' - ' . get_date($double_upload_end_time, 'DATE');
    } else {
        $htmlout .= $dstatus;
    }
    $htmlout .= ' ]
    ...
    $htmlout .= "
                                <div class='level is-marginless'>
                                    <span>Silverleech</span><span> [ ";
    if ($half_down_enabled) {
        $htmlout .= '<span class="has-text-success"> ACTIF</span> ' . get_date($half_down_start_time, 'DATE') . ' - ' . get_date($half_down_end_time, 'DATE');
    } else {
        $htmlout .= $hstatus;
    }
    $htmlout .= ' ]
