...
function torrenttable($res, $variant = 'index')
{
    ...
    $htmlout .= ($variant === 'index' ? "
                    <th class='has-text-centered tooltipper' title='Aller à mes marque-page'>
    ...
    if ($CURUSER['class'] >= UC_STAFF) {
        $htmlout .= "
                    <th class='has-text-success w-1 tooltipper' title='Tools'>Options</th>";
    }
    $htmlout .= '
            </tr>
        </thead>
    ...
    while ($row = mysqli_fetch_assoc($res)) {
                            ...
                            <div class='tooltip_templates'>
                                <span id='torrent_{$row['id']}_tooltip'>
                                    <div class='is-flex tooltip-torrent'>
                                        ...
                                        <span class='margin10'>
                                            <b class='size_4 right10 has-text-primary'>Nom:</b>" . htmlsafechars($dispname) . "<br>
                                            <b class='size_4 right10 has-text-primary'>Ajouté le:</b>" . get_date($row['added'], 'DATE', 0, 1) . "<br>
                                            <b class='size_4 right10 has-text-primary'>Taille</b>" . mksize(htmlsafechars($row['size'])) . "<br>
                                            <b class='size_4 right10 has-text-primary'>Sous-titre:</b> {$Subs}<br>
                                            <b class='size_4 right10 has-text-primary'>Seedeurs</b>" . (int) $row['seeders'] . "<br>
                                            <b class='size_4 right10 has-text-primary'>Leecheurs</b>" . (int) $row['leechers'] . "<br>
                                        </span>
                                    </div>
                                </span>
                            </div>
                    ...
                    <div class='level-center'>
                        <div class='flex-inrow'>
                            <a href='{$site_config['baseurl']}/download.php?torrent={$id}" . ($CURUSER['ssluse'] == 3 ? '&amp;ssl=1' : '') . "' class='flex-item'>
                                <i class='icon-download icon tooltipper' title='Télécharger le torrent!'></i>
                            </a>
                        </div>
                    </div>
                </td>
                        ...
                        <div class='level-center'>
                            <div class='flex-inrow'>
                                <a href='{$site_config['baseurl']}/bookmark.php?torrent={$id}&amp;action=add' class='flex-item bookmark' name='{$id}'>
                                    <i class='icon-ok icon has-text-success tooltipper' title='Mettre en marque-page!'></i>
                                </a>
                            </div>
                        </div>
                    </span>
                        ...
                        <div class='level-center'>
                            <div class='flex-inrow'>
                                <a href='{$site_config['baseurl']}/bookmark.php?torrent={$id}&amp;action=delete' class='flex-item remove' name='{$id}'>
                                    <i class='icon-ok icon has-text-red tooltipper' title='Supprimer le marque-page!'></i>
                                </a>
                            </div>
                        </div>
                    </span>";
        ...
        if ($CURUSER['class'] >= UC_STAFF) {
            ...
            $edit_link = "
                <span>
                    <a href='{$site_config['baseurl']}/edit.php?id=" . (int) $row['id'] . "{$returnto}' class='tooltipper' title='Edition rapide'>
                        <i class='icon-edit icon'></i>
                    </a>
                </span>";
            $del_link = ($CURUSER['class'] === UC_MAX ? "
                <span>
                    <a href='{$site_config['baseurl']}/fastdelete.php?id=" . (int) $row['id'] . "{$returnto}' class='tooltipper' title='Suppression rapide'>
